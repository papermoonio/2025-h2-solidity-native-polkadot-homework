...
let factory;
  ...
  it('createPair:identical', async function() {
    let tokenAddress = await token.getAddress();
    await expect(factory.createPair(tokenAddress, tokenAddress))
      .to.be.revertedWith('UniswapV2: IDENTICAL_ADDRESSES');
  });

  it('createPair:zeroAddress', async function() {
    let tokenAddress = await token.getAddress();
    // Test when tokenA is zero address
    await expect(factory.createPair(ethers.ZeroAddress, tokenAddress))
      .to.be.revertedWith('UniswapV2: ZERO_ADDRESS');
    // Test when tokenB is zero address
    await expect(factory.createPair(tokenAddress, ethers.ZeroAddress))
      .to.be.revertedWith('UniswapV2: ZERO_ADDRESS');
    // Test when both are zero address
    await expect(factory.createPair(ethers.ZeroAddress, ethers.ZeroAddress))
      .to.be.revertedWith('UniswapV2: IDENTICAL_ADDRESSES');
  });
