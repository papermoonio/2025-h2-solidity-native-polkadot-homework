...
async function main() {
  ...
  const contractFile = 'ERC20Mintable.sol'; // 如果你的合约文件名不同，请修改
  const contractName = 'ERC20Mintable';     // 如果你的合约名不同，请修改
  ...
  const contract = await factory.deploy("erc20", "ERC20", 18, ethers.parseUnits("1000000", 18));

  const tx = contract.deploymentTransaction();
  console.log('部署交易发送中，tx:', tx?.hash);

  await contract.waitForDeployment();
  console.log('合约已部署到地址:', await contract.getAddress());
}
