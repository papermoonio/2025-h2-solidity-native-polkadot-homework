...
describe("DelegateCall", async function () {
  it("Should increment the counter by 1 when calling the callInc() function", async function () {
    ...
    const proxy = await ethers.deployContract("Proxy", [logic.getAddress()]);
    await proxy.waitForDeployment();
    const counter = await proxy.counter();
    console.log("counter", counter);
    assert.equal(counter, 100n, "initial counter should be 100");
    await proxy.callInc();
    const counter2 = await proxy.counter();
    console.log("counter2", counter2);
    assert.equal(counter2, counter + 1n, "counter should increment by 1");
  });
});