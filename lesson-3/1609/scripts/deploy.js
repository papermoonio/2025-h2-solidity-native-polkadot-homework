...
async function main() {
    console.log("开始部署MintableERC20合约...");
    ...
    const token = await MintableERC20.deploy("Mintable Token", "MINT");

    await token.waitForDeployment();

    const tokenAddress = await token.getAddress();
    console.log("MintableERC20 合约已部署到:", tokenAddress);
    ...
    console.log("总供应量:", ethers.formatEther(await token.totalSupply()), "MINT");
    console.log("部署者余额:", ethers.formatEther(await token.balanceOf(deployer.address)), "MINT");
    console.log("合约owner:", await token.owner());
